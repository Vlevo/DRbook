...
To handle moving the player, we need to keep track of the position across the game loops. This lets us know where the player was last `tick`, check if they should move, and then update their position accordingly.

Update your `mygame/app/main.rb` to this:
...
`||=` in English is "or equals". That code above, `args.state.player_x ||= 120` would be read as: assign the property `player_x` on `args.state` the numeric value of 120 unless it's already been assigned a value.
...
Then, finally, we change the hardcoded `x` and `y` values of the dragon sprite to be the changeable values stored in `args.state` so that we can actually make use of those values instead of being stuck at our hard-coded position.
...
```
Then we check for vertical movement. We increase `player_y` to move up, decrease it to move down.
...
It's so important that this happens after checking for input. You don't want to change `args.state.player_x` after this check, as the boundary won't be enforced. Order matters with the code we write within `tick`.
